...
class hpatch_descr:
    ...
    itr = tps
    def __init__(self,base=None,descr_type='',sep=',', descr_map=None):
        if base is not None:
            self.base = base
...
class hpatch_sequence:
    ...
    def __init__(self,base=None):
        ...
        for t in self.itr:
            ...
            data = np.split(im, self.N)
            data = np.concatenate(data).reshape(len(data), im.shape[1], im.shape[1])
