from hpatches.utils.misc import *
import dill
import pprint
import os.path
...
def results_verification(desc,splt, res=None):
    ...
    for r in v:
        ...
        for t in ['e','h','t']:
            if r == 'imbalanced':
                aps.append(res[t]['inter'][r][v[r]])
                aps.append(res[t]['intra'][r][v[r]])
            local_results.append([ft[t], res[t]['inter'][r][v[r]],res[t]['intra'][r][v[r]]])
        print(tb(local_results,headers=heads))

    return np.mean(aps), tb(local_results,headers=heads)


def results_matching(desc,splt, res=None):
    if res is None:
    ...
    print("\n")
    return (mAP['e'] + mAP['h'] + mAP['t']) / (k_mAP * 3.0), tb([results],headers=heads)


def results_retrieval(desc,splt, res=None):
    if res is None:
    ...
    print(tb(results,headers=heads))
    return np.mean(results[-1][1:]), tb(results,headers=heads)
