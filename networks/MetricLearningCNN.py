...
class MetricLearningCNN(nn.Module):
    ...
    def pairwise_symmetric_gru(self, S1A, S1B):
        ...
        if self.arch_version == 1:
            ...
            gru_last_output1 = gru_last_output1[self.gru_layers - 1]
            ...
            output1 = L2Norm()(output1)
            output2 = L2Norm()(output2)
            gru_output1, _ = self.gru(output1)  # (384, 841, 128), _
            gru_output2, _ = self.gru(output2)
            gru_output1 = L2Norm()(gru_output1)
            gru_output2 = L2Norm()(gru_output2)
            attention_params1 = F.softmax(gru_output1 * output1, dim=1)
            attention_params2 = F.softmax(gru_output2 * output2, dim=1)
            output1 = output1 * attention_params1
            output2 = output2 * attention_params2
