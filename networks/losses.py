...
class OnlineContrastiveLoss(nn.Module):
    ...
    def forward(self, emb1, emb2):
        negative_indices = torch.randint(high=emb1.shape[0], size=(emb1.shape[0],), device=self.device)
        positive_loss = (emb1 - emb2).pow(2).sum(1)
        negative_loss = F.relu(
            self.margin - (emb1 - emb2[negative_indices]).pow(2).sum(
                1).sqrt()).pow(2)
