...
with h5py.File(train_mat_file_path, 'r') as f:
    ...
    train_data = train_data.transpose(0, 2, 3, 1).reshape(train_data.shape[0], 1, train_data.shape[2],
                                                          train_data.shape[3], train_data.shape[1])
...
with h5py.File(test_mat_file_path, 'r') as f:
    ...
    test_data = test_data.transpose(0, 2, 3, 1).reshape(test_data.shape[0], 1, test_data.shape[2], test_data.shape[3],
                                                        test_data.shape[1])
